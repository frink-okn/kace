...
**HDT conversion** (`workflows/hdt_conversion.py`, the big one): download inputs (skipped if `files_list` passed in) → `hdtc-job` (HDT create) → NT merge job (`hdtc dump | gzip`) → RIOT validation job → registry-configured augmentation steps (see below) → VOID job → append pav version/date triples to `void.nt` (version from `get_future_tag`) → per-KG QLever index job (`IndexBuilderMain`, with hardcoded per-repo grep filters for `secure-chain-kg` / `spatial-kg`) → upload hdt/nt/void/qlever artifacts to LakeFS → also upload `void.nt` to the `config.void_repo` (`okn-void:develop`) → review email to KG owners → cleanup. Jobs run via `run_k8s_job` + `watch_k8s_job_sync` pairs.

**Per-KG augmentations** (`temporal_app/augmentations.py`): additive-only, registry-configured triple generation inside the HDT conversion. KGs opt in via `frink-options.augmentations` in the okn-registry kgs.yaml — a list of `{name, params}` steps validated against `AUGMENTATION_CATALOG` (currently `map-predicate`: for every `(s, from, o)` in the built HDT emit `(s, to, o)`, e.g. `rdfs:label` from a name-like predicate; `from` may be a str or list). Each step runs as an `hdtc-job` (`hdtc search | sed | gzip`, needs the `.index.v1-1` from `create --index`) writing its own `aug/NN-name.nt.gz`; a final merge job runs `hdtc create graph.hdt <aug files> --index` (HDT inputs are supported by hdtc — dictionary reused, not reparsed) and gzip-appends the aug files to `nt/graph.nt.gz`, so VOID, the per-KG QLever index, LDF, and the federated build all pick up the new triples with no further changes. Steps can only ADD triples — destructive transformations are deliberately unsupported. New step types: add a builder + catalog entry in `augmentations.py` (pure string-building, safe to call from workflow code). Unknown names/bad params fail the workflow fast (Slack notify) before any job runs. Registry config changes alone do NOT retrigger conversion — re-fire the webhook endpoint.
