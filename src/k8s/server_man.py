...
class ServerDeploymentManager:
    ...
    def get_httproute(self, parameters: Dict[str, Any]) -> Dict:
        httproute_template = self.templates.get_template("httproute.j2")
        parameters.update({
    ...
    def get_healthcheck(self, parameters: Dict[str, Any]) -> Dict:
        healthcheck_template = self.templates.get_template("healthcheck.j2")
        return yaml.safe_load(healthcheck_template.render(parameters))
    ...
    def create_or_update_httproute(self, parameters: Dict[str, Any], annotations: Dict[str, str] = None) -> None:
        httproute_body = self.get_httproute(parameters)
        httproute_name = httproute_body["metadata"]["name"]
        # update annotations
        if annotations:
            raw_annotations = httproute_body["metadata"].get("annotations", {})
            raw_annotations.update(annotations)
        ...
        try:
            k8s_client.get_namespaced_custom_object(
                group=group, version=version, namespace=self.namespace, plural=plural, name=httproute_name
            )
            k8s_client.patch_namespaced_custom_object(
                group=group, version=version, namespace=self.namespace, plural=plural, name=httproute_name, body=httproute_body
            )
        except ApiException as e:
            if e.status == 404:
                k8s_client.create_namespaced_custom_object(
                    group=group, version=version, namespace=self.namespace, plural=plural, body=httproute_body
                )
            else:
                raise e
    ...
    def create_or_update_healthcheck(self, parameters: Dict[str, Any], annotations: Dict[str, str] = None) -> None:
        ...
        # update annotations if needed, though usually not critical for HealthCheckPolicy
        if annotations:
            raw_annotations = healthcheck_body["metadata"].get("annotations", {})
            raw_annotations.update(annotations)
        ...
        try:
            ...
            if e.status == 404:
                k8s_client.create_namespaced_custom_object(
                    group=group, version=version, namespace=self.namespace, plural=plural, body=healthcheck_body
                )
            else:
                raise e
    ...
    def delete_httproute_k8s(self, parameters: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if e.status == 404:
                logger.info("HTTPRoute not found: {0}".format(httproute_name))
            else:
                raise e
    ...
    def delete_healthcheck_k8s(self, parameters: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if e.status == 404:
                logger.info("HealthCheckPolicy not found: {0}".format(healthcheck_name))
            else:
                raise e
    ...
    def are_all_services_running(self, parameters: Dict[str, Any], annotations: Dict[str, Any]) -> bool:
        ...
        # Ingress check removed; HTTPRoute status check to be added if needed.
