...
class HDTConversionWorkflow:
    ...
    async def run(self, action_payload: dict, doc_path: str,
        ...
        pav_ttl_path = f"{local_dir}/hdt/void.ttl"

        # PAV ontology prefixes:
        ...
        ttl_content = (
            ...
            f"<{dataset_uri}> pav:version \"{version_tag}\" .\n"
            f"<{dataset_uri}> pav:version \"{version}\" .\n"
            f"<{dataset_uri}> pav:lastUpdatedOn \"{timestamp_str}\"^^xsd:dateTime .\n"
            f"<{dataset_uri}> dct:modified \"{month_year_str}\" .\n"
        )

        await workflow.execute_activity(
            create_local_file,
            # file path , content, append
            args=[pav_ttl_path, ttl_content, True],
            start_to_close_timeout=timedelta(minutes=1),
            retry_policy=NO_RETRY
        )
        ...
        qlever_cmd = (
            ...
            f"-f <(gunzip -c {working_dir}/nt/graph.nt.gz) "
            f"-g https://purl.org/okn/frink/kg/{kg_title} -F nt "
            f"-f {working_dir}/hdt/void.ttl "
            f"-g void -F ttl  --stxxl-memory {mem_size}"
